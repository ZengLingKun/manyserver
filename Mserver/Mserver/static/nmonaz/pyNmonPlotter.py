    ...
    def plotStats(self, todoList, isAIX=False):
        ...
        for stat, fields, plotOpts in todoList:
            ...
            if "CPU001" in stat:
                # parse NMON date/timestamps and produce datetime objects
                times = [datetime.datetime.strptime(
                    d, "%d-%b-%Y %H:%M:%S") for d in self.processedData["CPU001"][0][1:]]
                values = []
                values.append((self.processedData["CPU001"][1][1:], "usr"))
                values.append((self.processedData["CPU001"][2][1:], "sys"))
                values.append((self.processedData["CPU001"][3][1:], "wait"))

                data = (times, values)
                fname = self.plotStat(data, xlabel="Time", ylabel="CPU001 load (%)",
                                      title="CPU001 vs Time", isPrct=True, stacked=True)
                outFiles.append(fname)
                ...
                for i in self.processedData["NET"]:
                    colTitle = i[:1][0]
                    for iface in fields:
                        if colTitle.startswith(iface) and "read" in colTitle:
                            read = np.array([float(x) for x in i[1:]])
                            values.append((read, colTitle))

                        elif colTitle.startswith(iface)  and "write" in colTitle:
                ...
                read2 = np.array([])
                write2 = np.array([])
                for i in self.processedData["DISKREAD"]:
                    colTitle = i[:1][0]
                    #with open('/tmp/djq_colTitle','a') as f:f.write(colTitle+'\n')
                    for iface in fields:
                        if "sda" == colTitle:
                            read1 = np.array([float(x) for x in i[1:]])
                            values.append((read1, colTitle+':r'))
                            break
                        else:
                          if "sdb" == colTitle:
                             read2 = np.array([float(x) for x in i[1:]])
                             values.append((read2, colTitle+':r'))
                             break
                          

                for i in self.processedData["DISKWRITE"]:
                    colTitle = i[:1][0]
                    for iface in fields:
                        if "sda" == colTitle:
                            write1 = np.array([float(x) for x in i[1:]])
                            values.append((write1, colTitle+':w'))
                            break
                        else:
                          if "sdb" == colTitle:
                             write2 = np.array([float(x) for x in i[1:]])
                             values.append((write2, colTitle+':w'))
                             break

                data = (times, values)
                max_=max(np.hstack((read1,read2,write1,write2)))
                fname = self.plotStat(data, xlabel="Time", ylabel="DISKREAD-WRITE KB/s",
                                      title="DISKREAD-DISKWRITE vs Time", yrange=[0, max_ * 1.2])
                outFiles.append(fname)
                ...
                RunQueue = np.array([])
                Blocked = np.array([])
                for i in self.processedData["PROC"]:
                    colTitle = i[:1][0]
                    for iface in fields:
                        if "Runnable" == colTitle:
                            RunQueue = np.array([float(x) for x in i[1:]])
                            values.append((RunQueue, colTitle))

                        elif "Blocked" == colTitle:
                            Blocked = np.array([float(x) for x in i[1:]])
                            values.append((Blocked, colTitle))

                data = (times, values)
                fname = self.plotStat(data, xlabel="Time", ylabel="Quenens /s",
                                      title="Quenens vs Time", yrange=[0, max(max(RunQueue), max(Blocked)) * 1.2])
                outFiles.append(fname)
                ...
                pswpin = np.array([])
                pswpout = np.array([])
                for i in self.processedData["VM"]:
                    colTitle = i[:1][0]
                    for iface in fields:
                        if "pswpin" == colTitle:
                            pswpin = np.array([float(x) for x in i[1:]])
                            values.append((pswpin, colTitle))

                        elif "pswpout" == colTitle:
                            pswpout = np.array([float(x) for x in i[1:]])
                            values.append((pswpout, colTitle))

                data = (times, values)
                max_ = max(max(pswpout), max(pswpin))
                if max_==0:max_=1
                fname = self.plotStat(data, xlabel="Time", ylabel="SWAP-SPACE KB/s",
                                      title="SWAP-SPACE vs Time", yrange=[0.01, max_ * 1.2])
                outFiles.append(fname)
